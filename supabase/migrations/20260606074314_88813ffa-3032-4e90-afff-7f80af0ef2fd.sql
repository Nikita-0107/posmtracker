...
BEGIN
  ...
  bal_agg AS (
    SELECT bal.wd_code, bal.tl_id,
           SUM(GREATEST(bal.qty,0))::int AS stock_units,
           COUNT(*) FILTER (WHERE bal.qty > 0)::int AS material_types
    FROM bal
    GROUP BY bal.wd_code, bal.tl_id
  )
  SELECT
    htl.wd_code::text,
    w.wd_name::text,
    ...
    (CASE
      ...
      ELSE 'Inactive'
    END)::text,
    (SELECT max(x) FROM unnest(d.d) x)::date,
    COALESCE(public._compute_streak_from_dates(d.d, today_d), 0)::int,
    COALESCE(b.stock_units, 0)::int,
    COALESCE(b.material_types, 0)::int
  FROM htl
