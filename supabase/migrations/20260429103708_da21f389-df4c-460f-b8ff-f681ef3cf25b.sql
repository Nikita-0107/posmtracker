...
declare
  ...
  _name text;
  _qty integer;
  _item_batch public.batch_type;
  _item_batch_text text;
...
begin
  ...
  for _item in select * from jsonb_array_elements(_items) loop
    ...
    _item_batch_text := nullif(btrim(_item->>'batch_type'), '');
    ...
    if _item_batch_text is not null then
      begin
        _item_batch := _item_batch_text::public.batch_type;
      exception when invalid_text_representation then
        raise exception 'Invalid batch type % for material %', _item_batch_text, _code;
      end;
    else
      _item_batch := _batch_type;
    end if;
    ...
    values (
      ...
      _ref, _proof, _rdate, _item_batch, _receive_id
    );
  end loop;
...
begin
  ...
  if not exists (select 1 from public.materials where code = _code) then
    if _name is null then
      raise exception 'Material description is required for new material %', _code;
    end if;
    insert into public.materials (code, name) values (_code, _name);
  end if;
