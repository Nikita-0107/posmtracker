...
BEGIN
  ...
  WITH tl_dates AS (
    ...
    LEFT JOIN public.wd_tls t
      ON t.wd_code = h.wd_code AND t.tl_name = h.tl_name
  )
  SELECT
    w.ae_id,
    ae.ae_name,
...
REVOKE EXECUTE ON FUNCTION public.get_tl_activity_report(int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_tl_activity_report(int) TO authenticated, service_role;
